# code modified from CompareGAN repo. See https://github.com/google/compare_gan.
...
def weight_initializer(initializer="glorot_normal", stddev=0.02):
  ...
  if initializer == "glorot_normal":
    # originally was keras.initializers.glorot_normal()
    # but with the update to tensorflow 2.0
    # keras initializers were changed, and the above call
    # produces the error "__call__() got an unexpected 
    # keyword argument 'partition_info'". The fix is to
    # call tf.compat.v1.keras.whatever instead of
    # keras.whatever
...
def downsample_block(inputs, downsample, learnable_sc, kernel_size, output_dim1, output_dim2, use_sn=True, use_bias=True, activation=tf.nn.relu, name="downsample_block"):
  if inputs == None:
    return lambda x: downsample_block(x, name)
  else:
    h = inputs
    h = conv2d(h, output_dim1, kernel_size, kernel_size, 1, 1, stddev=0.02, name=name + ".1",
           use_sn=use_sn, use_bias=use_bias)
    h = activation(h)
    h = conv2d(h, output_dim2, kernel_size, kernel_size, 1, 1, stddev=0.02, name=name + ".2",
           use_sn=use_sn, use_bias=use_bias)
    h = activation(h)
    if downsample:
      h = tf.nn.avg_pool(h, 2, 2, padding="SAME")
    
    s = inputs
    if learnable_sc:
      s = conv2d(s, output_dim2, 1, 1, 1, 1, stddev=0.02, name=name + ".3",
           use_sn=use_sn, use_bias=use_bias)
      if downsample:
        s = tf.nn.avg_pool(s, 2, 2, padding="SAME")

    return h + s

def optimized_block(inputs, downsample, learnable_sc, kernel_size, output_dim, use_sn=True, use_bias=True, activation=tf.nn.relu, name="optimized_block"):
  if inputs == None:
    return lambda x: optimized_block(x, name)
  else:
    h = inputs
    h = conv2d(h, output_dim, kernel_size, kernel_size, 1, 1, stddev=0.02, name=name + ".conv.01",
           use_sn=use_sn, use_bias=use_bias)
    h = activation(h)
    h = conv2d(h, output_dim, kernel_size, kernel_size, 1, 1, stddev=0.02, name=name + ".conv.02",
           use_sn=use_sn, use_bias=use_bias)
    h = activation(h)
    h = tf.nn.avg_pool(h, 2, 2, padding="SAME")
    
    s = inputs
    s = conv2d(s, output_dim, 1, 1, 1, 1, stddev=0.02, name=name + ".conv.03",
         use_sn=use_sn, use_bias=use_bias)

    return h + s

def upsample_block(inputs, is_training, upsample, learnable_sc, kernel_size, output_dim1, output_dim2, use_sn=True, use_bias=True, activation=tf.nn.relu, name="upsample_block"):
  if inputs == None:
    return lambda x: upsample_block(x, name)
  else:
    h = inputs
    if upsample:
      h = deconv2d(h, in_channels, 2, 2, 2, 2, 0.02, name + ".deconv.01", use_sn)
    h = conv2d(h, output_dim1, kernel_size, kernel_size, 1, 1, stddev=0.02, name=name + ".conv.01",
           use_sn=use_sn, use_bias=use_bias)
    h = batch_norm(h, is_training, True, True, name + ".bn.01")
    h = activation(h)
    h = conv2d(h, output_dim2, kernel_size, kernel_size, 1, 1, stddev=0.02, name=name + ".conv.02",
           use_sn=use_sn, use_bias=use_bias)
    h = batch_norm(h, is_training, True, True, name + ".bn.02")
    h = activation(h)

    s = inputs
    if learnable_sc:
      if upsample:
        s = deconv2d(s, in_channels, 2, 2, 2, 2, 0.02, name + ".deconv.02", use_sn)
      h = conv2d(h, output_dim1, 1, 1, 1, 1, stddev=0.02, name=name + ".conv.01",
           use_sn=use_sn, use_bias=use_bias)
